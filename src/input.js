    ...
    document
      ...
      .addEventListener(
        ...
        event => {
          ...
          if(unit?.team==="fighter"&&!window.GameBattle.waitingForTarget&&!window.GameBattle.runningTurn){window.GameRenderer.showInspect(unit);return}
          window.GameBattle.selectCombatant(id);
        }
      );

    document.getElementById("inspectClose")?.addEventListener("click",()=>window.GameRenderer.hideInspect());
    document.getElementById("collectionButton")?.addEventListener("click",()=>window.GameRenderer.showCollection());
    document.getElementById("collectionClose")?.addEventListener("click",()=>window.GameRenderer.hideCollection());
