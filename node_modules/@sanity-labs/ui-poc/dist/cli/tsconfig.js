 ...
 */
export function resolveTargetTsconfig(cwd) {
    const root = join(cwd, 'tsconfig.json');
    if (!existsSync(root))
        return { file: root, exists: false };
    let data;
    try {
        data = readJsonc(root);
    }
...
function effectiveCompilerOptions(file, seen = new Set()) {
 ...
 */
export function planTsconfigChanges(effective) {
    const changes = [];
    const opts = effective;
    const jsx = opts['jsx'];
    if (!jsx)
        changes.push({ key: 'jsx', from: undefined, to: 'react-jsx', load: true });
    ...
    if (lib === undefined) {
        ...
        if (!hasEsBaseline(lib))
            missing.push(ES_BASELINE);
        if (missing.length > 0) {
...
export function inspectTsconfig(cwd) {
 ...
 */
export function reconcileTsconfig(cwd, { dryRun = false } = {}) {
    const target = resolveTargetTsconfig(cwd);
    if (!target.exists) {
        if (!dryRun) {
    ...
    if (!dryRun) {
        ...
        const options = doc.compilerOptions ?? {};
        doc.compilerOptions = options;
        for (const change of changes) {
            if (change.merge && change.key === 'lib') {
                const current = Array.isArray(options['lib']) ? options['lib'] : [];
                options['lib'] = [...current, ...change.to];
            }
            else {
