...
function printPlan(cwd, detected, plan) {
    heading('Plan');
...
export async function runInit(options) {
    ...
    info(color.dim(`framework: ${detected.framework} · package manager: ${detected.packageManager} · entry: ${detected.entry} · typescript: ${detected.typescript}`));
    ...
    if (install.ok) {
        ...
        info(`  ${color.dim('hint:')} This is usually a peer-dependency conflict. Re-run the install and read the conflict, or retry with your package manager's legacy peer-deps flag.`);
    }
    if (plan.typeSpecs.length > 0) {
        step('Installing React type definitions…');
        const typeInstall = runInstall(cwd, detected.packageManager, plan.typeSpecs, { dev: true });
        if (typeInstall.ok)
            success('Type definitions installed.');
        else
            warn(`Could not install ${plan.typeSpecs.join(' ')}. Add them manually.`);
    }
    if (plan.stylesheet.exists && !plan.stylesheet.imported) {
        const wired = wireStylesheet(cwd, detected.entry, { dryRun: false });
        if (wired.wrote)
            success(`Imported the stylesheet in ${detected.entry}.`);
    }
    ...
    if (detected.typescript) {
        ...
        if (result.created)
            success('Created tsconfig.json with required options.');
        else if (result.unreadable)
            warn('tsconfig.json could not be parsed; left untouched.');
        else if (result.changes.length === 0)
            success('tsconfig already has the required options.');
        else
            success(`Updated tsconfig: ${result.changes.map((c) => c.key).join(', ')}.`);
    }
    ...
    fail(`${failed.length} check${failed.length === 1 ? '' : 's'} failed. Run \`npx ${PACKAGE_NAME} doctor\` for details.`);
    return 1;
}
