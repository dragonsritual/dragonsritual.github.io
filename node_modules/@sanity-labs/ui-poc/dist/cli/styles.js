...
const PRESENT = new RegExp(String.raw `(?:^|\n)\s*import\s+(?:[^\n'"]*\bfrom\s+)?['"]${SPEC}['"]` +
...
const MAX_FILES = 800;
export function hasStylesImport(content) {
...
function findImportInDir(root, budget) {
    const stack = [root];
 ...
 */
export function inspectStylesheet(cwd, entry) {
    const file = join(cwd, entry);
    const exists = existsSync(file);
    if (exists && fileImports(file))
        return { entry, file, exists, imported: true };
...
function injectIntoAstro(content) {
...
export function wireStylesheet(cwd, entry, { dryRun = false } = {}) {
    const file = join(cwd, entry);
    if (!existsSync(file))
        return { entry, file, missing: true, imported: false, wrote: false };
