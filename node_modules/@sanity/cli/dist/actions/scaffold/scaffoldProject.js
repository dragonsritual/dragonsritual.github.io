...
function createScaffoldEnv(dataset, projectId, token) {
    return {
...
export async function scaffoldProject({ cancelSignal, dataset, displayName, output, packageManager, projectId, telemetry, token, workDir }) {
    ...
    writeScaffoldEnv(path.join(studioPath, '.env.local'), scaffoldEnv, `Studio scaffold completed, but writing ./${STUDIO_ENV_FILE} failed`);
    if (detected) {
    ...
    writeScaffoldEnv(path.join(frontendPath, '.env.local'), scaffoldEnv, `Website scaffold completed, but writing ./${FRONTEND_ENV_FILE} failed`);
...
function writeScaffoldEnv(envPath, values, errorContext) {
    const lines = [
