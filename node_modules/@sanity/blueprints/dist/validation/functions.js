...
function validateFunctionEventResourceDataset(event) {
    ...
    if (!('resource' in event) || typeof event.resource === 'undefined')
        return [];
 ...
 * @returns Array of validation errors, empty if valid
 */
function validateFunctionContentLakeEvent(event) {
    if (!event || typeof event !== 'object')
        return [{ type: 'invalid_type', message: '`event` must be an object' }];
    if (!('type' in event))
        return [{ type: 'missing_parameter', message: '`event.type` is required' }];
    switch (event.type) {
        case 'document':
 ...
 * @returns Array of validation errors, empty if valid
 */
export function validatePubSubFunction(functionResource) {
    if (!functionResource)
        return [{ type: 'invalid_value', message: 'Function config must be provided' }];
    if (typeof functionResource !== 'object')
        return [{ type: 'invalid_type', message: 'Function config must be an object' }];
 ...
 * @returns Array of validation errors, empty if valid
 */
export function validateDurableFunction(functionResource) {
    if (!functionResource)
        return [{ type: 'invalid_value', message: 'Function config must be provided' }];
    if (typeof functionResource !== 'object')
        return [{ type: 'invalid_type', message: 'Function config must be an object' }];
