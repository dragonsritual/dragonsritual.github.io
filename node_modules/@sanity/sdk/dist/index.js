...
import "groq";
import { getPublishedId, DocumentId, getVersionId, getDraftId, isDraftId, isVersionId, isPublishedId } from "@sanity/id-utils";
...
function createSanityInstance(config = {}) {
  ...
  }, logger2 = createLogger("sdk", { instanceContext });
  logger2.info("Sanity instance created", {
    hasProjectId: !!config.projectId,
    hasDataset: !!config.dataset,
    hasAuth: !!config.auth,
    hasPerspective: !!config.perspective
  }), logger2.debug("Instance configuration", {
    projectId: config.projectId,
    dataset: config.dataset,
  ...
  const instance = {
    ...
    dispose: () => {
      if (disposed.current) {
        logger2.trace("Dispose called on already disposed instance", { internal: !0 });
        return;
      }
      logger2.trace("Disposing instance", {
        internal: !0,
        listenerCount: disposeListeners.size
      }), disposed.current = !0, disposeListeners.forEach((listener) => listener()), disposeListeners.clear(), logger2.info("Instance disposed");
    },
    onDispose: (cb) => {
    ...
    createChild: (next) => {
      logger2.debug("Creating child instance", {
      ...
      return logger2.trace("Child instance created", {
        internal: !0,
...
const handleAuthCallback = bindActionGlobally(
  ...
  async ({ state, instance }, locationHref = getDefaultLocation()) => {
    const logger2 = getAuthLogger(instance), { providedToken, callbackUrl, clientFactory, apiHost, storageArea, storageKey } = state.get().options;
    if (providedToken)
      return logger2.debug("Skipping auth callback - token already provided"), !1;
    const { authState } = state.get();
    if (authState.type === AuthStateType.LOGGING_IN && authState.isExchangingToken)
      return logger2.debug("Skipping auth callback - token exchange already in progress"), !1;
    const cleanedUrl = getCleanedUrl(locationHref), tokenFromUrl = getTokenFromLocation(locationHref);
    if (tokenFromUrl)
      return logger2.info("Auth token found in URL, logging in"), state.set("setTokenFromUrl", {
        authState: createLoggedInAuthState(tokenFromUrl, null)
      }), cleanedUrl;
    const authCode = getAuthCode(callbackUrl, locationHref);
    if (!authCode)
      return logger2.debug("No auth code found in callback URL"), !1;
    ...
    try {
      ...
      if (contextParam) {
        ...
        parsedContext && typeof parsedContext == "object" && (delete parsedContext.sid, dashboardContext = parsedContext, logger2.debug("Dashboard context parsed from callback URL", {
          hasDashboardContext: !0
        }));
      }
    } catch (err) {
      logger2.warn("Failed to parse dashboard context from callback URL", { error: err });
    }
    logger2.info("Exchanging auth code for token"), state.set("exchangeSessionForToken", {
    ...
    try {
      ...
      logger2.debug("Fetching token from auth endpoint");
      ...
      return logger2.info("Auth token obtained successfully, user logged in"), storageArea?.setItem(storageKey, JSON.stringify({ token })), state.set("setToken", { authState: createLoggedInAuthState(token, null) }), cleanedUrl;
    } catch (error) {
      return logger2.error("Failed to exchange auth code for token", { error }), state.set("exchangeSessionForTokenError", { authState: { type: AuthStateType.ERROR, error } }), cleanedUrl;
    }
  }
), logout = bindActionGlobally(authStore, async ({ state, instance }) => {
  const logger2 = getAuthLogger(instance), { clientFactory, apiHost, providedToken, storageArea, storageKey } = state.get().options;
  if (providedToken) {
    logger2.debug("Skipping logout - token is statically provided");
    return;
  }
  const { authState } = state.get();
  if (authState.type === AuthStateType.LOGGED_OUT && authState.isDestroyingSession) {
    logger2.debug("Skipping logout - already in progress");
    return;
  }
  const token = authState.type === AuthStateType.LOGGED_IN && authState.token;
  try {
    if (token) {
      logger2.info("Logging out user"), state.set("loggingOut", {
      ...
      logger2.debug("Calling logout endpoint"), await client.request({ uri: "/auth/logout", method: "POST", tag: "logout" });
    } else
      logger2.debug("No token to logout - already logged out");
  } catch (error) {
    throw logger2.error("Logout request failed", { error }), error;
  } finally {
    logger2.info("User logged out, clearing stored tokens"), state.set("logoutSuccess", {
...
), CONNECT_RETRY_MAX_DELAY = 1e3 * 240;
...
const handleIncomingMessage = (event) => {
  ...
  return defer(() => {
    ...
    retry$1({
      delay: (_error, retryCount) => timer(Math.min(CONNECT_RETRY_MAX_DELAY, 2 ** retryCount * 100)),
      resetOnSuccess: !0
    }),
...
}, createIncomingEvents = (bifur, connections$) => connections$.pipe(
  switchMap$1(
    () => bifur.listen("presence").pipe(
  ...
  share$1()
), createUnload = () => typeof window > "u" ? EMPTY : merge(fromEvent(window, "beforeunload"), fromEvent(window, "pagehide")).pipe(
  map(() => {
  })
), createBifurTransport = (options) => {
  const { client, token$, sessionId } = options, bifur = getBifurClient(client, token$), connections$ = createConnections(bifur);
  return [createIncomingEvents(bifur, connections$), (message) => {
    switch (message.type) {
      case "rollCall":
  ...
  }, connections$, createUnload()];
};
function isKeyedSegment(segment) {
  return typeof segment == "object" && segment !== null && !Array.isArray(segment) && "_key" in segment;
}
...
const logger = createLogger("presence"), PRESENCE_API_VERSION = "2026-03-30", REPORT_MIN_INTERVAL = 3e4, REPORT_AUDIT_TIME = 200, SESSION_TTL = 9e4, SWEEP_INTERVAL = 15e3, getInitialState = () => ({
  locations: /* @__PURE__ */ new Map(),
  users: {}
}), sendSafely = (dispatch2, message) => dispatch2(message).pipe(
  catchError$1((error) => (logger.warn("Failed to send presence message", { messageType: message.type, error }), EMPTY))
), asTrigger = (source$, name) => source$.pipe(
  map$1(() => {
  }),
  catchError$1((error) => (logger.error("Presence announce trigger failed", { trigger: name, error }), EMPTY))
), locationKey = ({ documentId, path, selection }) => JSON.stringify([documentId, path, selection ?? null]);
...
const presenceStore = defineStore({
  ...
  initialize: (context) => {
    ...
    subscription.add(
      ...
      connections$.pipe(switchMap(() => timer(SWEEP_INTERVAL, SWEEP_INTERVAL))).subscribe(() => {
        state.set("presence/expire", (prevState) => {
          const cutoff = Date.now() - SESSION_TTL, stale = [...prevState.locations].filter(([, s]) => s.lastSeenAt < cutoff);
      ...
      unload$.pipe(switchMap(() => sendSafely(dispatch2, { type: "disconnect" }))).subscribe()
    );
    const localLocations$ = state.observable.pipe(
      map$1((s) => s.localLocations),
    ...
    ), rollCallRequests$ = incomingEvents$.pipe(
      ...
      filter$1((event) => event.type === "rollCall" && event.sessionId !== sessionId)
    );
    subscription.add(
      merge(
        ...
        map$1(([, locations]) => locations),
        // Stay silent until the app has reported at least once.
        filter$1((locations) => locations !== void 0),
        auditTime(REPORT_AUDIT_TIME),
        switchMap((locations) => sendSafely(dispatch2, { type: "state", locations }))
      ).subscribe()
    );
    const userIds$ = state.observable.pipe(
      map$1(
        (s) => Array.from(s.locations.values()).map((l) => l.userId).filter((id) => !!id)
      ),
      distinctUntilChanged((a, b) => a.length === b.length && a.every((v, i) => v === b[i]))
    ), organizationId$ = isCanvasResource(resource) ? state.observable.pipe(
      filter$1((s) => s.organizationId !== void 0 || s.organizationIdError !== void 0),
      first$1(),
      map$1((s) => s.organizationId)
    ) : of(void 0);
    if (subscription.add(
      combineLatest([userIds$, organizationId$]).pipe(
...
function matchesQuery(location, target, { path, excludeVersions }) {
  return scopeId(location.documentId, excludeVersions) !== target ? !1 : !path || startsWithPath(path, pathOf(location));
}
...
function computeDocumentPresence(state, options) {
  ...
  return Array.from(state.locations).flatMap(
    ([sessionId, session]) => session.locations.filter((location) => matchesQuery(location, target, options)).map((location) => toDocumentPresence(state.users, sessionId, session.userId, location))
  );
}
...
const _getDocumentPresence = bindActionByResource(
  presenceStore,
  createStateSourceAction({
    selector: ({ state }, options) => selectDocumentPresence(state, options)
  })
);
...
const _reportPresence = bindActionByResource(
  presenceStore,
  ({ state }, { locations }) => {
    const lastActiveAt = (/* @__PURE__ */ new Date()).toISOString(), wireLocations = locations.map((location) => ({
      type: "document",
