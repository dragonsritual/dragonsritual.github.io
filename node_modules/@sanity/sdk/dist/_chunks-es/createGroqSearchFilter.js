import { createClient, CorsOriginError, DisconnectError, ConnectionFailedError } from "@sanity/client";
import { createImageUrlBuilder } from "@sanity/image-url";
import "groq";
import { Observable, map, distinctUntilChanged, shareReplay, skip, defer, finalize, filter, exhaustMap, timer, switchMap, takeWhile, catchError, EMPTY, fromEvent, retry, BehaviorSubject, combineLatest, startWith, tap, pairwise, mergeMap, groupBy, of, share, NEVER, first, firstValueFrom, race } from "rxjs";
import "@sanity/types";
...
const REFRESH_INTERVAL = 720 * 60 * 1e3;
...
const LIVE_EVENTS_API_VERSION = "v2025-05-06", LIVE_EVENTS_RETRY_DELAY = 1e3;
      ...
      (client) => defer(
        ...
        catchError((error) => {
          if (error instanceof CorsOriginError)
            return onCorsError(error), EMPTY;
          if (error instanceof DisconnectError)
            return EMPTY;
          if (error instanceof ConnectionFailedError && typeof error.status == "number" && error.status >= 400 && error.status < 500)
            return EMPTY;
          throw error;
        }),
...
const RELEASES_QUERY = "releases::all()", RELEASES_API_VERSION = "v2025-05-06";
function observeReleases(instance, { resource, onCorsError }) {
  return getClientState(instance, {
    ...
    switchMap((client) => {
      const syncTags$ = new BehaviorSubject(void 0);
      return combineLatest([
        observeLiveEvents(instance, { resource, onCorsError }).pipe(startWith(void 0)),
        syncTags$
      ]).pipe(
        filter(
          ([message, syncTags]) => message === void 0 || message.tags.some((tag) => syncTags?.includes(tag))
        ),
        map(([message]) => message?.id),
        distinctUntilChanged()
      ).pipe(
        switchMap(
          (lastLiveEventId) => client.observable.fetch(
            RELEASES_QUERY,
            {},
            {
              perspective: "raw",
...
function sortReleases(releases = []) {
  return [...releases].sort((a, b) => {
    const aType = a.metadata?.releaseType, bType = b.metadata?.releaseType;
...
), getAllReleasesState = (instance, options) => _getAllReleasesState(instance, options ?? {}), subscribeToReleases = ({
  instance,
  state,
  key: { resource }
}) => observeReleases(instance, {
  resource,
...
).subscribe({ error: (error) => state.set("setError", { error }) }), DEFAULT_PERSPECTIVE = "drafts", optionsCache = /* @__PURE__ */ new Map(), selectInstancePerspective = (context, _) => context.instance.config.perspective, selectActiveReleases = (context) => context.state.activeReleases, selectOptions = (_context, options) => options, memoizedOptionsSelector = createSelector(
...
), getPerspectiveState = (instance, ...rest) => (
  // bindActionByResource keyFn destructures { resource } from the first param, so pass {} when no options
...
const _resolveQuery = bindActionByResource(
  ...
  ({ state, instance }, { signal, ...options }) => {
    const normalized = normalizeOptionsWithPerspective(instance, options), { getCurrent } = getQueryState(instance, normalized), key = getQueryKey(normalized), subscriptionId = insecureRandomId();
    state.set("addSubscriber", addSubscriber(key, subscriptionId));
    const aborted$ = signal ? new Observable((observer) => {
      const cleanup = () => {
    ...
    ) : NEVER, resolved$ = state.observable.pipe(
      map(getCurrent),
      first((i) => i !== void 0)
    ), promise = firstValueFrom(race([resolved$, aborted$])), releaseSubscriber = () => {
      setCleanupTimeout(() => {
        state.set("removeSubscriber", removeSubscriber(key, subscriptionId));
      }, QUERY_STATE_CLEAR_DELAY);
    };
...
var version = "2.19.0";
const CORE_SDK_VERSION = getEnv("PKG_VERSION") || `${version}-development`, API_VERSION = "vX", PROJECT_API_VERSION = "2025-07-18", USERS_STATE_CLEAR_DELAY = 5e3, DEFAULT_USERS_BATCH_SIZE = 100, getUsersKey = (instance, {
  resourceType,
  organizationId,
