...
function getInitials({ id, name }) {
  const index = id.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0) % POSSIBLE_HUES.length;
  return {
...
function resolveInitials(name) {
  if (!name) return "";
  const namesArray = name.replace(SYMBOLS, "").split(WHITESPACE).filter(Boolean);
...
}), AssetSourceInterfaceSchema = InterfaceBase.extend({
...
function toBrettInterfaces(interfaces) {
  return (interfaces ?? []).map(
    ({ src: _src, ...deployed }) => deployed
  );
}
function createStudioApplicationFromLocal(local) {
  const { manifest } = local, firstWorkspaceProjectId = manifest?.workspaces[0]?.projectId, projectId = brandProjectId(
    local.projectId ?? firstWorkspaceProjectId ?? `${local.host}-${local.port}`
  ), id = brandApplicationId(local.id ?? `${local.host}-${local.port}`), interfaces = toBrettInterfaces(local.interfaces), title = manifest?.workspaces[0]?.title ?? `${local.host}:${local.port}`;
  return {
    id,
    type: "studio",
    title,
...
function createCoreApplicationFromLocal(local) {
  const { manifest } = local, id = brandApplicationId(local.id ?? `${local.host}-${local.port}`), interfaces = toBrettInterfaces(local.interfaces);
  return {
    id,
    type: "coreApp",
...
}, CONFIG_MODULE_EXPOSE = "configs/installation_config";
