...
const require_getScopedTheme = require("./getScopedTheme.cjs");
let react_compiler_runtime = require("react-compiler-runtime"), react = require("react");
react = __toESM(react, 1);
let styled_components = require("styled-components"), react_jsx_runtime = require("react/jsx-runtime"), react_is = require("react-is"), _sanity_icons_Spinner = require("@sanity/icons/Spinner"), _floating_ui_react_dom = require("@floating-ui/react-dom"), motion_react = require("motion/react"), _juggle_resize_observer = require("@juggle/resize-observer"), react_dom = require("react-dom"), _sanity_icons_Close = require("@sanity/icons/Close"), use_effect_event = require("use-effect-event"), _sanity_icons_ChevronRight = require("@sanity/icons/ChevronRight");
/**
* @public
* @deprecated Use `createColorTheme` from `@sanity/ui/theme` instead.
*/
const createColorTheme = require_getScopedTheme.a, hexToRgb = require_getScopedTheme.d, hslToRgb = require_getScopedTheme.f, multiply = require_getScopedTheme.c, parseColor = require_getScopedTheme.u, rgbToHex = require_getScopedTheme.p, rgbToHsl = require_getScopedTheme.m, rgba = require_getScopedTheme.l, screen = require_getScopedTheme.s, studioTheme = require_getScopedTheme.o(), EMPTY_ARRAY = [], EMPTY_RECORD = {}, POPOVER_MOTION_DURATION = .2, POPOVER_MOTION_PROPS = {
	card: {
		initial: {
			scale: .97,
...
function _getResponsiveSpace(theme, props, spaceIndexes = EMPTY_ARRAY) {
	...
	let { media, space } = require_getScopedTheme.i(theme);
...
function useTheme_v2() {
	...
	return $[0] === t0 ? t1 = $[1] : (t1 = require_getScopedTheme.i(t0), $[0] = t0, $[1] = t1), t1;
}
...
function responsiveFlexGapStyle(props) {
	let { media, space } = require_getScopedTheme.i(props.theme);
...
const StyledFlex = /* @__PURE__ */ (0, styled_components.styled)(Box).withConfig({
...
function responsiveFont(fontKey, props) {
	let { $size, $weight } = props, { font, media } = require_getScopedTheme.i(props.theme), { family, sizes, weights } = font[fontKey], fontWeight = $weight && weights[$weight] || weights.regular, defaultSize = sizes[2], base = {
		position: "relative",
		fontFamily: family,
...
function buttonBaseStyles(props) {
	let { $width } = props, { style } = require_getScopedTheme.i(props.theme);
...
function buttonColorStyles(props) {
	let { $mode } = props, { button, color: baseColor, style } = require_getScopedTheme.i(props.theme);
	props.$mode;
...
function border(props) {
	let { card, media } = require_getScopedTheme.i(props.theme), borderStyle = `${card.border?.width ?? 1}px solid var(--card-border-color)`;
...
function responsiveShadowStyle(props) {
	let { card, media, shadow } = require_getScopedTheme.i(props.theme);
...
function ThemeProvider(props) {
	...
	bb1: {
		...
		let t2;
		$[4] !== rootTheme || $[5] !== scheme || $[6] !== tone ? (t2 = require_getScopedTheme.t(rootTheme, scheme, tone), $[4] = rootTheme, $[5] = scheme, $[6] = tone, $[7] = t2) : t2 = $[7], t1 = t2;
	}
	let theme = t1;
	if (!theme) {
		let t2;
...
function cardBaseStyle(props) {
	let { $checkered } = props, { space } = require_getScopedTheme.i(props.theme);
...
function cardColorStyle(props) {
	let { $checkered, $focusRing, $muted } = props, { card, color, style } = require_getScopedTheme.i(props.theme), border = {
...
function responsiveInputPaddingStyle(props) {
	let { $fontSize, $iconLeft, $iconRight, $padding, $space } = props, { font, media, space } = require_getScopedTheme.i(props.theme), len = Math.max($padding.length, $space.length, $fontSize.length), _padding = [], _space = [], _fontSize = [];
...
function textInputBaseStyle(props) {
	let { $scheme, $tone, $weight } = props, { color, font } = require_getScopedTheme.i(props.theme);
...
function textInputFontSizeStyle(props) {
	let { font, media } = require_getScopedTheme.i(props.theme);
...
function textInputRepresentationStyle(props) {
	let { $hasPrefix, $hasSuffix, $scheme, $tone, $unstableDisableFocusRing } = props, { color, input } = require_getScopedTheme.i(props.theme);
...
const CLEAR_BUTTON_BOX_STYLE = { zIndex: 2 }, StyledTextInput = /* @__PURE__ */ (0, styled_components.styled)(Card).attrs({ forwardedAs: "span" }).withConfig({
...
function useClickOutsideEvent(listener, t0, boundaryElement) {
	let $ = (0, react_compiler_runtime.c)(9), elementsArg = t0 === void 0 ? _temp$5 : t0, t1;
...
}), StyledHotkeys = styled_components.styled.kbd.withConfig({
...
function selectableColorStyle(props) {
	let { $tone } = props, { color, style } = require_getScopedTheme.i(props.theme), tone = color.selectable[$tone];
...
function PortalProvider(props) {
	let $ = (0, react_compiler_runtime.c)(7), { boundaryElement, children, element, __unstable_elements: elements } = props, fallbackElement = (0, react.useSyncExternalStore)(emptySubscribe, _temp, _temp2), t0 = boundaryElement || null, t1 = element || fallbackElement, t2;
